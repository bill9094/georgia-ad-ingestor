...
class FCCClient:
    def __init__(self,cfg):
        self.base=cfg.get('base_url','https://publicfiles.fcc.gov').rstrip('/'); self.delay=float(cfg.get('request_delay_seconds',0.75)); self.timeout=int(cfg.get('timeout_seconds',30)); self.page_size=int(cfg.get('history_page_size',100)); self.session=requests.Session(); self.session.headers['User-Agent']=cfg.get('user_agent','GeorgiaPoliticalAdResearch/1.0'); self._last=0.0
    def _wait(self):
    ...
    def _results(payload):
        ...
        for k in ('results','data','items','files','facilities'):
            v=payload.get(k)
            if isinstance(v,list): return v
            if isinstance(v,dict):
                for kk in ('results','items','files','facilities'):
                    if isinstance(v.get(kk),list): return v[kk]
        return []
    def search_tv_facilities(self,state='GA'): return self._results(self.get(f'/api/service/tv/facility/search/{state}.json').json())
