...
def effective_amount(rec,doc_type):
    # Invoices are accounting evidence only when an explicit invoice-level total was extracted.
    # Never fall back to gross/net/rate fields, which can be per-spot values.
    return rec.get('invoice_total') if doc_type=='invoice' else (rec.get('contract_total') or rec.get('net_amount') or rec.get('gross_amount'))
