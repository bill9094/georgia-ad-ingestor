...
def _resolve_exact_history_document(client, q):
    ...
    service_candidates = []
    if q['service']:
        service_candidates.append(q['service'])
    service_candidates.extend([None, 'cable', 'tv', 'dbs'])
    seen = set()
    matches = []
    for service in service_candidates:
        ...
        seen.add(service)
        try:
            docs = list(client.file_history(str(entity_id), service, since, until))
        except Exception:
            continue
        exact = [d for d in docs if (d.file_name or '').strip() == str(file_name).strip()]
        if exact:
            matches.extend(exact)
            break
    unique = {(d.folder_id, d.file_manager_id): d for d in matches if d.folder_id and d.file_manager_id}
    if not unique:
        raise LookupError(f'FCC history did not resolve exact filename for entity {entity_id}: {file_name}')
    if len(unique) > 1:
        raise LookupError(f'FCC history returned multiple exact matches for entity {entity_id}: {file_name}')
...
def _download_exact(client, q, dest):
    ...
    if folder_id and file_manager_id:
        ...
        local, url = client.download(doc, dest)
        return local, url, doc
    ...
    for q in rows:
        ...
        try:
            ...
            con.execute('''INSERT INTO documents(entity_id,folder_id,file_manager_id,file_name,create_ts,last_update_ts,
                        ...
                        (q['entity_id'],resolved_folder,resolved_file,q['file_name'],resolved_doc.create_ts or q['discovered_at'],resolved_doc.last_update_ts or q['discovered_at'],resolved_doc.source_service_code or q['service'],final_url,
            ...
            if ex['doc_type'] in ('contract','invoice'):
                upsert_order(con,did,q['entity_id'] or '',q['file_name'] or '',ex['doc_type'],rec,resolved_doc.create_ts or q['discovered_at'])
                stats['reconciled'] += 1
