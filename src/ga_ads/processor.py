...
def _history_window(discovered_at):
    ...
    try:
        d=datetime.fromisoformat(str(discovered_at).replace('Z','+00:00'));return (d-timedelta(days=3)).date().isoformat(),(d+timedelta(days=3)).date().isoformat()
    except Exception:return None,None

def _resolve_exact_history_document(client,q):
    if not q['entity_id'] or not q['file_name']:raise ValueError('Exact FCC history resolution requires entity_id + file_name')
    since,until=_history_window(q['discovered_at']);return client.find_exact_file(str(q['entity_id']),str(q['file_name']),q['service'],since,until)

def _download_exact(client,q,dest):
    if q['folder_id'] and q['file_manager_id']:
        doc=FCCDocument(str(q['entity_id'] or ''),str(q['folder_id']),str(q['file_manager_id']),q['file_name'],None,q['discovered_at'],q['discovered_at'],None,None,q['service']);local,url=client.download(doc,dest);return local,url,doc
    if q['entity_id'] and q['file_name']:
        doc=_resolve_exact_history_document(client,q);local,url=client.download(doc,dest);return local,url,doc
    url=q['source_url']
    if not url:raise ValueError('Queued document needs folder_id + file_manager_id, entity_id + exact file_name, or an exact source_url')
    r=requests.get(url,timeout=client.timeout,headers={'User-Agent':client.session.headers.get('User-Agent','GeorgiaPoliticalAdResearch/1.0')});r.raise_for_status()
    if 'pdf' not in (r.headers.get('content-type') or '').lower() and not r.content.startswith(b'%PDF'):raise ValueError(f'Exact source_url did not return PDF content: {r.url}')
    Path(dest).parent.mkdir(parents=True,exist_ok=True);Path(dest).write_bytes(r.content);return str(dest),r.url,FCCDocument(str(q['entity_id'] or ''),'','',q['file_name'],None,q['discovered_at'],q['discovered_at'],None,None,q['service'])
...
def process_queue(cfg,limit=100,reprocess=False):
    ...
    statuses="'queued','retry'" if not reprocess else "'queued','retry','reconciled','needs_visual_review'"
    ...
    for q in rows:
        ...
        try:
            ...
            did=con.execute('SELECT id FROM documents WHERE folder_id=? AND file_manager_id=?',(resolved_folder,resolved_file)).fetchone()['id'];rec=ex['record']
            ...
            needs_review=bool(ex['needs_visual_review']) or (ex['doc_type']=='contract' and not any(rec.get(k) is not None for k in ('contract_total','net_amount','gross_amount')));status='needs_visual_review' if needs_review else 'reconciled'
            if needs_review:
