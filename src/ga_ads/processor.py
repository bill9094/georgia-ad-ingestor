...
def _metadata_advertiser(note):
    ...
    s=str(note).strip().rstrip('/').split('/')[-1].strip()
    s=re.sub(r'^(?:AMP\s*-\s*|MHB\s*)','',s,flags=re.I).strip()
    return s or None
...
def _resolve_exact_history_document(client,q):
    ...
    return client.find_exact_file(str(q['entity_id']),str(q['file_name']),q['service'],since,until)


def _download_exact(client,q,dest):
    if q['folder_id'] and q['file_manager_id']:
        doc=FCCDocument(str(q['entity_id'] or ''),str(q['folder_id']),str(q['file_manager_id']),q['file_name'],None,q['discovered_at'],q['discovered_at'],None,None,q['service'])
    ...
    return str(dest),r.url,FCCDocument(str(q['entity_id'] or ''),'','',q['file_name'],None,q['discovered_at'],q['discovered_at'],None,None,q['service'])
    ...
    for q in rows:
        ...
        try:
            ...
            if not rec.get('advertiser'):
                ...
                if fallback:
                    rec['advertiser']=fallback;rec['advertiser_source']='fcc_index_folder_metadata'
