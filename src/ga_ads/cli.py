...
def main():
    p=argparse.ArgumentParser(prog='ga-ads')
    p.add_argument('--config',default='config/georgia.yaml')
    ...
    pq=sp.add_parser('process-queue'); pq.add_argument('--limit',type=int,default=100)
    sp.add_parser('queue-status')
    sp.add_parser('audit-status')
    ...
    if args.cmd=='init-db':
        init_db(resolve(cfg,'storage.sqlite_path')); result={'status':'ok'}
    elif args.cmd=='import-queue':
        result=import_document_queue(cfg,args.path)
    elif args.cmd=='import-audit':
        result=import_audit_signals(cfg,args.path)
    elif args.cmd=='process-queue':
        result=process_queue(cfg,args.limit)
    elif args.cmd=='queue-status':
        result=queue_status(cfg)
    elif args.cmd=='audit-status':
        result={'unresolved':unresolved_audit(cfg)}
    elif args.cmd=='weekly':
        result=weekly(cfg,args.since,args.until)
    elif args.cmd=='publish':
        result=publish_snapshot(cfg,args.since,args.until)
    else:
        result={'removed':cleanup(cfg,args.hours)}
